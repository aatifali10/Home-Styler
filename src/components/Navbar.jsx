...
const Navbar = () => {
  ...
  return (
    <nav className={`w-full h-20 flex items-center justify-between ${isDarkMode ? 'bg-gray-800' : 'bg-white'} fixed z-50 px-4 lg:px-16 shadow`}>
      <Link to="/" className="flex items-center gap-x-1">
        <CgIfDesign className={`text-3xl ${isDarkMode ? 'text-yellow-500' : 'text-yellow-600'}`} />
        <h1 className={`uppercase font-semibold text-xl ${isDarkMode ? 'text-white' : 'text-black'}`}>HOME DECOR</h1>
      </Link>
        ...
        <li className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <ul className="absolute left-0 w-40 bg-white shadow-lg py-1 z-10">
              ...
              <li><Link to="/decor" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Decor</Link></li>
              <li><Link to="/lighting" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Lighting</Link></li>
              <li><Link to="/kitchen" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Kitchen</Link></li>
              <li><Link to="/bathroom" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Bathroom</Link></li>
            </ul>
          )}
        </li>
        <li><Link to="/about-us" className={`text-base font-medium ${isDarkMode ? 'text-white' : 'text-black'} hover:text-yellow-500`}>About Us</Link></li>
      ...
      <button className="lg:hidden flex items-center" onClick={() => setOpenNav(!openNav)}>
        <IoMenu className={`text-xl ${isDarkMode ? 'text-yellow-500' : 'text-yellow-600'}`} />
      </button>

      {/* Mobile Menu */}
      <div className={`fixed top-0 left-0 right-0 bottom-0 z-20 flex justify-end ${openNav ? "translate-x-0" : "translate-x-full"} transition-transform duration-500`} onClick={() => setOpenNav(false)}>
        <div className={`bg-white w-3/5 md:w-2/5 h-full shadow-xl p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <button className="bg-yellow-500 w-8 h-8 flex items-center justify-center" onClick={() => setOpenNav(false)}>
